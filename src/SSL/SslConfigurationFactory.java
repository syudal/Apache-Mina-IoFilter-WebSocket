...
import SSL.impl.DefaultSslConfiguration;
import Util.IoUtils;
...
public class SslConfigurationFactory {
    ...
    private KeyStore loadStore(File storeFile, String storeType,
        ...
        try {
            if (storeFile.exists()) {
                System.out.println("[알림] Trying to load store from file");
                fin = new FileInputStream(storeFile);
            } else {
                System.out.println("[알림] Trying to load store from classpath");
                fin = getClass().getClassLoader().getResourceAsStream(storeFile.getPath());

                if (fin == null) {
                    throw new RuntimeException("Key store could not be loaded from " + storeFile.getPath());
                }
            }
    ...
    public SslConfiguration createSslConfiguration() {
        ...
        try {
            ...
            System.out.println("[알림] DefaultSsl.configure()");
            throw new RuntimeException("DefaultSsl.configure()", ex);
        }
    }
